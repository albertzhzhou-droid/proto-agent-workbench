# CLI and workflow guide

Detailed commands and workflow semantics moved from the repository overview.
For a shorter introduction, start with [Getting started](getting-started.md).
...
- `.codex/skills/` contains seven project-scoped AcademicForge adaptations. `proto-agent skills list|resolve|audit` and the read-only `proto_skills_list|resolve` MCP tools parse their bounded vendor-neutral manifests, resolve only declared CLI/MCP/HTTP interfaces, and never execute Skill content. See [`docs/academicforge_skill_adaptation.md`](academicforge_skill_adaptation.md).
