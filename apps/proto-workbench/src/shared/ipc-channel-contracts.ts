...
import { chatRequestLimit } from "./research-request-limits.ts";
...
export type IpcRequestChannel = Exclude<(typeof IPC)[keyof typeof IPC], IpcPushChannel>;
...
const MODULE_SETTINGS = z.object({
  ...
  enabledSkills: z.array(z.enum(DESIGN_SKILLS.map(skill => skill.id))).max(DESIGN_SKILLS.length).optional(),
  enabledOptional: z.array(z.enum(OPTIONAL_MODULES.map(module => module.id as OptionalModuleId))).max(OPTIONAL_MODULES.length),
}).strict();
...
export const IPC_ARGUMENT_SCHEMAS = {
  [IPC.chemOpen]: noArguments,
  [IPC.journalList]: z.tuple([z.object({surface:z.enum(["chat","harness","compute","workflow","design","chemistry","validation","figure","system","legacy"]).optional(),scopeId:z.string().min(1).max(256).optional(),state:z.enum(["intent","dispatched","no-effect","effect-unknown","completed","reconciled-applied","reconciled-not-applied"]).optional(),limit:z.number().int().min(1).max(200).optional(),offset:z.number().int().min(0).max(1000000).optional()}).strict()]),
  [IPC.journalInspect]: z.tuple([z.object({operationId:z.string().min(1).max(256)}).strict()]),
  [IPC.journalReconcile]: z.tuple([z.object({operationId:z.string().min(1).max(256),verdict:z.enum(["applied","not-applied"]),actor:z.string().trim().min(1).max(256),evidenceRef:z.string().trim().min(1).max(4096)}).strict()]),
  [IPC.chemScienceRequest]: z.tuple([z.record(z.string(),z.unknown()).refine(request=>new TextEncoder().encode(JSON.stringify(request)).byteLength<=2*1024*1024,'Chemistry request is too large.')]),
  [IPC.researchChat]: z.tuple([z.record(z.string(), z.unknown()).refine(request => new TextEncoder().encode(JSON.stringify(request)).byteLength <= chatRequestLimit(request), "Chat request is too large.")]),
  ...
  [IPC.modelsProbeTools]: z.tuple([ID]),
  [IPC.modelsEvaluationSummaries]: noArguments,
  ...
  [IPC.computeRun]: z.tuple([z.object({
    tool: z.string().regex(/^[a-z][a-z0-9_]*$/).max(100),
    arguments: z.record(z.string(), z.json()),
  }).strict().refine(value => JSON.stringify(value).length <= 1_000_000, "Computation input exceeds 1 MB.")]),
...
} satisfies Record<IpcRequestChannel, z.ZodType<unknown[]>>;

/** One schema table validates input and supplies handler/preload argument tuples. */
export type InferChannelArgs<C extends IpcRequestChannel> = z.output<(typeof IPC_ARGUMENT_SCHEMAS)[C]>;
export type InferChannelInput<C extends IpcRequestChannel> = z.input<(typeof IPC_ARGUMENT_SCHEMAS)[C]>;
export type IpcHandler<C extends IpcRequestChannel, R = unknown> = (...args: InferChannelArgs<C>) => R | Promise<R>;

export function validateChannelArguments<C extends IpcRequestChannel>(channel: C, args: unknown[]): InferChannelArgs<C>;
export function validateChannelArguments(channel: string, args: unknown[]): unknown[];
export function validateChannelArguments(channel: string, args: unknown[]): unknown[] {
  const schema = (IPC_ARGUMENT_SCHEMAS as Record<string, z.ZodType<unknown[]>>)[channel];
...
/** Method-to-channel bindings shared by native preload and browser mock typing. */
export const IPC_API_CHANNELS = {
  journal: {list: IPC.journalList, inspect: IPC.journalInspect, reconcile: IPC.journalReconcile},
  ...
  harness: {listExecutions: IPC.harnessExecutions, resumeExecution: IPC.harnessResume, pauseExecution: IPC.harnessPause, preflight: IPC.harnessPreflight, simulatePolicy: IPC.harnessPolicySimulation, previewDecisionBundle: IPC.harnessDecisionBundlePreview, exportDecisionBundle: IPC.harnessDecisionBundleExport, verifyDecisionBundles: IPC.harnessDecisionBundleVerify, previewTrustPolicy: IPC.harnessTrustPolicyPreview, exportTrustPolicy: IPC.harnessTrustPolicyExport, listTrustPolicies: IPC.harnessTrustPolicyList, importSignatureEvidence: IPC.harnessSignatureEvidenceImport, listSignatureEvidence: IPC.harnessSignatureEvidenceList, importTrustRootCandidate: IPC.harnessTrustRootCandidateImport, listTrustRootCandidates: IPC.harnessTrustRootCandidateList, importTransparencyWitnessPack: IPC.harnessTransparencyWitnessImport, listTransparencyWitnessPacks: IPC.harnessTransparencyWitnessList},
  ...
  runs: {list: IPC.runsList, cockpit: IPC.runsCockpit, searchEvidence: IPC.runsSearchEvidence, get: IPC.runsGet, getDetail: IPC.runsGetDetail, createCheckpoint: IPC.runsCreateCheckpoint, previewResume: IPC.runsPreviewResume, forkCheckpoint: IPC.runsForkCheckpoint, archive: IPC.runsArchive},
  reviews: {get: IPC.reviewsGet, updateChecklist: IPC.reviewsUpdateChecklist, addComment: IPC.reviewsAddComment, listComments: IPC.reviewsListComments, approve: IPC.reviewsApprove},
  approvals: {list: IPC.approvalsList, resolve: IPC.approvalsResolve},
} as const;

/** Return contracts retain the existing domain types while request arguments
 * come exclusively from the schemas above. Event subscription methods stay local. */
type BoundMethod<D extends keyof typeof IPC_API_CHANNELS, M extends keyof (typeof IPC_API_CHANNELS)[D]> =
  D extends keyof WorkbenchApi ? M extends keyof NonNullable<WorkbenchApi[D]> ? NonNullable<WorkbenchApi[D]>[M] : never : never;
type ChannelMethod<C extends IpcRequestChannel> = {
  [D in keyof typeof IPC_API_CHANNELS]: {
    [M in keyof (typeof IPC_API_CHANNELS)[D]]: (typeof IPC_API_CHANNELS)[D][M] extends C ? BoundMethod<D, M> : never
  }[keyof (typeof IPC_API_CHANNELS)[D]]
}[keyof typeof IPC_API_CHANNELS];
export type InferChannelResult<C extends IpcRequestChannel> = ChannelMethod<C> extends (...args: never[]) => infer R ? Awaited<R> : never;
type ContractDomain<D extends keyof WorkbenchApi> = {
  [M in keyof NonNullable<WorkbenchApi[D]>]: D extends keyof typeof IPC_API_CHANNELS
    ? M extends keyof (typeof IPC_API_CHANNELS)[D]
      ? (typeof IPC_API_CHANNELS)[D][M] extends infer C extends IpcRequestChannel
        ? C extends typeof IPC.researchChat | typeof IPC.chemScienceRequest
          ? NonNullable<WorkbenchApi[D]>[M]
          : (...args: InferChannelInput<C>) => Promise<InferChannelResult<C>> : NonNullable<WorkbenchApi[D]>[M]
      : NonNullable<WorkbenchApi[D]>[M]
    : NonNullable<WorkbenchApi[D]>[M]
};
/** Both implementations are checked against the same inferred argument tuples. */
export type IpcWorkbenchApi = { [D in keyof WorkbenchApi]: ContractDomain<D> };

export function defineChannel<const C extends IpcRequestChannel, A extends z.ZodType<unknown[]>, R extends z.ZodType>(name: C, args: A, result: R) {
  return {name, args, result};
}

// Legacy response domains retain their domain TypeScript contracts. Runtime
// field validation is explicit rather than pretending z.unknown validates them.
const JOURNAL_RECORD = z.object({
  operationId:z.string(), runId:z.string(), scope:z.object({surface:z.string(),scopeId:z.string(),parentOperationId:z.string().optional()}),
  tool:z.string(), capabilityId:z.string().nullable().optional(), effect:z.enum(["read","write"]), argumentsSha256:SHA256,
  state:z.enum(["intent","dispatched","no-effect","effect-unknown","completed","reconciled-applied","reconciled-not-applied"]),
  outcome:z.enum(["ok","tool-error"]).optional(), decisionId:z.string().optional(), decision:z.record(z.string(),z.unknown()).optional(),
...
export function validateChannelResult<C extends IpcRequestChannel>(channel:C,result:unknown):InferChannelResult<C> {
  return IPC_CHANNEL_CONTRACTS[channel].result.parse(result);
}

/**
 * Compile-time cross-check between the schema table and the domain contract.
 *
 * Preload and the browser mock take their argument tuples from the schemas
 * above, so they follow a schema change instead of drifting from it. The main
 * process handler and `WorkbenchApi` are still written by hand, so this is where
 * a schema can silently stop matching what the rest of the application believes
 * it accepts. Every channel's inferred input must remain acceptable to its
 * domain method; dropping a field the domain requires fails here.
 *
 * The two streaming channels keep their domain signature because they carry
 * callbacks that no argument schema describes, exactly as `ContractDomain` does.
 *
 * This checks that the schema promises no more than the domain accepts. The
 * opposite direction — a domain type looser than the boundary, as with
 * `ComputeRequest.arguments` typed `unknown` where the boundary takes only JSON
 * values — is not enforced here; tightening those domain types is separate work.
 */
type StreamingChannel = typeof IPC.researchChat | typeof IPC.chemScienceRequest;
type ContractDomainMethod<D extends keyof typeof IPC_API_CHANNELS, M extends keyof (typeof IPC_API_CHANNELS)[D]> =
  D extends keyof WorkbenchApi ? M extends keyof NonNullable<WorkbenchApi[D]> ? NonNullable<WorkbenchApi[D]>[M] : never : never;
type ChannelsDivergingFromDomain = { [D in keyof typeof IPC_API_CHANNELS]: { [M in keyof (typeof IPC_API_CHANNELS)[D]]:
  ContractDomainMethod<D, M> extends (...args: infer A) => unknown
    ? (typeof IPC_API_CHANNELS)[D][M] extends infer C extends IpcRequestChannel
      ? C extends StreamingChannel ? never : InferChannelInput<C> extends A ? never : `${D & string}.${M & string}`
      : never
    : never }[keyof (typeof IPC_API_CHANNELS)[D]] }[keyof typeof IPC_API_CHANNELS];
/** Distributes the union so a failure names the offending channels in the error. */
type NameDivergingChannels<T> = T extends unknown ? T : never;
const EVERY_CHANNEL_MATCHES_ITS_DOMAIN_CONTRACT: "no channel diverges" =
  null as unknown as NameDivergingChannels<ChannelsDivergingFromDomain>;
export type { ChannelsDivergingFromDomain };
export const ipcContractCrossCheck = EVERY_CHANNEL_MATCHES_ITS_DOMAIN_CONTRACT;
